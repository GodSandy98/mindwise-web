    ...
    <div className="max-w-3xl mx-auto p-6">
      {student && (
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">{student.class_name} · ID {student.id}</p>
        </div>
      )}

      <div className="flex items-center gap-3 mb-6">
        <label className="text-sm text-gray-600">选择考试：</label>
        <select
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 uppercase text-xs">
              <tr>
                ...
                <th className="px-4 py-3 text-left">原始分</th>
                <th className="px-4 py-3 text-left">标准化得分</th>
                <th className="px-4 py-3 text-left">等级</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {scores.indicator_scores.map(s => (
                <tr key={s.indicator_id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-medium">{indicatorMap[s.indicator_id] ?? `指标${s.indicator_id}`}</td>
